...
function generateWildcardRequire(dir, wildcardPath, wildcardParam, wildcardBlocks, log) {
    ...
    let options = glob.sync(wildcardPrefix + '**' + wildcardSuffix + endPattern, {
        mark: true,
        ignore: 'node_modules/**/*'
    });
...
module.exports = async function (content, map) {
    ...
    const emitAssetDirectory = (wildcardPath, wildcards) => {
        ...
        const files = glob.sync(assetDirPath + wildcardPattern, {
            mark: true,
            ignore: 'node_modules/**/*'
        }).filter(name =>
    ...
    function emitStaticChildAsset(wrapRequire = false) {
        if (isAbsolutePathStr(staticChildValue.value)) {
            let resolved;
            try {
                resolved = path.resolve(staticChildValue.value);
            } catch (e) {
            }
            let emitAsset;
            ...
            let resolvedThen;
            try {
                resolvedThen = path.resolve(staticChildValue.then);
            } catch (e) {
            }
            let resolvedElse;
            try {
                resolvedElse = path.resolve(staticChildValue.else);
            } catch (e) {
            }
            let emitAsset;
